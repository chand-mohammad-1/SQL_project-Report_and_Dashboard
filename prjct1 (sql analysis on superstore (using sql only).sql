...
-- Database Size
SELECT pg_size_pretty(pg_database_size('sql_analysis'));
...
select product_id,Product_Name,ROUND(CAST((sales-profit) AS NUMERIC), 2)as cost
from store
...
/* Same Thing Using normal method without creating any temporary data. Here, This can be only viewed for one time and we can't merge with the current dataset in this process.*/
select  product_id,order_id,((profit/((sales-profit))*100)) as percentage_profit
from store
...
select distinct  (Customer_ID), Order_ID,city, Postal_Code
from store
where Customer_Id is not null;

/* Calculate Total Frequency of each order id by each customer Name in descending order. */
select order_id, customer_id, count(Order_Id) as total_order_id
from store
group by order_id,customer_id
order by total_order_id desc

/* Calculate  cost of each customer name. */
select order_id, customer_id, City, Quantity,sales,(sales-profit) as costs,profit
from store
...
SELECT Returned_items, Count(Returned_items) As Returned_Items_Count, Customer_ID,Customer_duration, States,City
FROM store
